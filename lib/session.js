...
export function getCwdHash(cwd) {
  const resolved = cwd.startsWith('~')
    ? cwd.replace(/^~/, homedir())
    : cwd;
  // Replace path separators and dots with '-' (matches Claude Code's directory naming)
...
export function findLatestSession(configDir, cwd) {
  ...
  try {
    ...
    for (const entry of entries) {
      // New format: UUID directory (Claude Code v2+)
      if (SESSION_ID_PATTERN.test(entry)) {
        const fullPath = join(projectDir, entry);
        try {
          const stat = statSync(fullPath);
          if (stat.isDirectory() && stat.mtimeMs > latestMtime) {
            latestMtime = stat.mtimeMs;
            latest = { sessionId: entry, path: fullPath };
          }
        } catch {
          // Skip
        }
...
export function findSessionAcrossProfiles(accounts, sessionId) {
  ...
  for (const account of accounts) {
    ...
    for (const cwdHash of cwdHashes) {
      // New format: UUID directory
      const sessionDir = join(projectsDir, cwdHash, sessionId);
      try {
        const stat = statSync(sessionDir);
        if (stat.isDirectory() && (!best || stat.mtimeMs > best.mtime)) {
          best = { account, cwdHash, path: sessionDir, mtime: stat.mtimeMs };
          continue;
        }
      } catch { /* not found */ }

      // Legacy format: UUID.jsonl file
      const sessionPath = join(projectsDir, cwdHash, `${sessionId}.jsonl`);
      try {
    ...
    for (const entry of entries) {
      ...
      if (SESSION_ID_PATTERN.test(entry)) {
        ...
        try {
          const stat = statSync(fullPath);
          if (stat.isDirectory() && (!best || stat.mtimeMs > best.mtime)) {
            best = { account, cwdHash, sessionId: entry, path: fullPath, mtime: stat.mtimeMs };
          }
        } catch { /* skip */ }
        continue;
      }
...
export function migrateSessionByHash(fromConfigDir, toConfigDir, cwdHash, sessionId) {
  ...
  try {
    ...
    const fromSessionJsonl = join(fromProjectDir, `${sessionId}.jsonl`);
    const fromSessionDir = join(fromProjectDir, sessionId);
    const hasDirEntry = existsSync(fromSessionDir) && statSync(fromSessionDir).isDirectory();
    const hasJsonl = existsSync(fromSessionJsonl);

    // New format: UUID directory WITHOUT a .jsonl sibling
    // (Legacy format has both a .jsonl file and a same-name directory for tool-results)
    if (hasDirEntry && !hasJsonl) {
      cpSync(fromSessionDir, join(toProjectDir, sessionId), { recursive: true, force: true });
      return { success: true };
    }

    // Legacy format: UUID.jsonl file (+ optional tool-results directory)
    if (!hasJsonl) {
      return { success: false, error: `Session file not found: ${fromSessionJsonl}` };
    }

    cpSync(fromSessionJsonl, join(toProjectDir, `${sessionId}.jsonl`), { force: true });
...
export function migrateSession(fromConfigDir, toConfigDir, cwd, sessionId) {
  ...
  try {
    ...
    // New format: UUID directory WITHOUT a .jsonl sibling
    if (hasDirEntry && !hasJsonl) {
      const toSessionDir = join(toProjectDir, sessionId);
      cpSync(fromSessionDir, toSessionDir, { recursive: true, force: true });
      return { success: true };
    }
    ...
    // Copy tool-results directory if it exists (legacy format only)
    const fromToolResults = join(fromProjectDir, sessionId);
    if (existsSync(fromToolResults)) {
      cpSync(fromToolResults, join(toProjectDir, sessionId), { recursive: true, force: true });
    }
