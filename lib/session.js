...
export function migrateSessionByHash(fromConfigDir, toConfigDir, cwdHash, sessionId) {
  ...
  try {
    ...
    if (existsSync(fromSessionDir) && statSync(fromSessionDir).isDirectory()) {
      cpSync(fromSessionDir, join(toProjectDir, sessionId), { recursive: true, force: true });
      return { success: true };
    }

    // Legacy format: UUID.jsonl file
    const fromSession = join(fromProjectDir, `${sessionId}.jsonl`);
    const toSession = join(toProjectDir, `${sessionId}.jsonl`);
