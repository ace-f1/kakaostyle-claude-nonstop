 ...
 *
 * Always uses the hash-based format "Claude Code-credentials-{hash}" to match
 * current Claude Code CLI behavior (which uses hashes for all dirs, including
 * the default ~/.claude).
 */
...
/**
 * Legacy service name (no hash) used by older Claude Code versions for ~/.claude.
 * Used as a fallback when reading credentials.
 */
export function getLegacyServiceName(configDir) {
  const expanded = expandPath(configDir);
 ...
 * Tries the current hash-based service name first, then falls back to the
 * legacy no-hash name for ~/.claude (older Claude Code versions).
 */
function readFromMacKeychain(configDir) {
  ...
  try {
    ...
    // Try legacy service name as fallback (only for ~/.claude)
    const legacyName = getLegacyServiceName(configDir);
    if (legacyName && error?.status === 44) {
      try {
        const raw = execFileSync('security', [
          'find-generic-password',
          '-s', legacyName,
  ...
  if (isMacOS()) {
    try {
      ...
      if (legacyName && error?.status === 44) {
        try {
          return execFileSync('security', [
            'find-generic-password', '-s', legacyName, '-a', account, '-w'
          ], { encoding: 'utf-8', timeout: 5000 }).trim();
        } catch {
          // Fall through
        }
