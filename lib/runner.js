...
import { pickBestAccount, effectiveUtilization, findFailbackAccount } from './scorer.js';
import { findLatestSession, findLatestSessionAcrossProfiles, migrateSession, migrateSessionByHash } from './session.js';
import { reauthExpiredAccounts } from './reauth.js';
import { CONFIG_DIR } from './config.js';
import { getCurrentTmuxSession } from './tmux.js';
import { saveRateLimitHit, clearRateLimitHit, applyRateLimitState } from './rate-limit-state.js';
...
/** Grace period after spawn before rate limit detection activates (ms).
 *  Prevents false positives from historical output re-rendered on --resume. */
const RATE_LIMIT_GRACE_MS = 10_000;
...
/** Whether proactive failback is enabled by default. CLI flags can override this. */
const AUTO_FAILBACK_ENABLED = process.env.CLAUDE_NONSTOP_AUTO_FAILBACK === '1'
  && process.env.CLAUDE_NONSTOP_DISABLE_FAILBACK !== '1';
/** Poll interval for checking whether a higher-priority account recovered. */
const FAILBACK_POLL_MS = Number.parseInt(process.env.CLAUDE_NONSTOP_FAILBACK_POLL_MS || '', 10) || 30 * 60_000;
/** Idle window before failback may interrupt the active Claude process. */
const FAILBACK_IDLE_MS = Number.parseInt(process.env.CLAUDE_NONSTOP_FAILBACK_IDLE_MS || '', 10) || 15_000;
/** Cooldown after any account switch before failback is allowed. */
const FAILBACK_COOLDOWN_MS = Number.parseInt(process.env.CLAUDE_NONSTOP_FAILBACK_COOLDOWN_MS || '', 10) || 30 * 60_000;
...
export async function run(claudeArgs, selectedAccount, allAccounts, options = {}) {
  ...
  const hasPriorities = allAccounts.some(a => a.priority != null);
  const autoFailback = options.autoFailback ?? (AUTO_FAILBACK_ENABLED && hasPriorities);
  console.error(`[claude-nonstop] Auto-failback: ${autoFailback ? 'enabled' : 'disabled'}`);
  ...
  while (swapCount <= maxSwaps) {
    const result = await runOnce(claudeArgs, currentAccount, sessionId, {
      ...
      allAccounts,
      autoFailback,
      failbackPollMs: options.failbackPollMs ?? FAILBACK_POLL_MS,
      failbackIdleMs: options.failbackIdleMs ?? FAILBACK_IDLE_MS,
      failbackCooldownMs: options.failbackCooldownMs ?? FAILBACK_COOLDOWN_MS,
      lastSwitchAt,
    });

    if (result.exitCode !== null && !result.rateLimitDetected && !result.failbackDetected) {
      // Normal exit — account ran successfully, clear any stale rate-limit state
      clearRateLimitHit(currentAccount.name);
      process.exitCode = result.exitCode;
      return;
    }

    if (!result.rateLimitDetected && !result.failbackDetected) {
      // Process ended without rate limit (e.g., signal) — clear stale state
      clearRateLimitHit(currentAccount.name);
    ...
    if (result.rateLimitDetected) {
      ...
      saveRateLimitHit(currentAccount.name, result.resetTime);
    ...
    } else if (result.failbackDetected) {
      console.error(`\n[claude-nonstop] Higher-priority account recovered while "${currentAccount.name}" was active. Failing back...`);
    }
    ...
    // Fallback: search across all registered accounts (handles sessions stored
    // in a different profile's configDir, e.g. ~/.claude default account)
    let crossProfileSession = null;
    if (!session) {
      crossProfileSession = findLatestSessionAcrossProfiles(allAccounts, cwd);
      if (crossProfileSession) {
        session = { sessionId: crossProfileSession.sessionId };
        console.error(`[claude-nonstop] Session found in account "${crossProfileSession.account.name}" (cross-profile fallback)`);
      }
    }
    ...
    let accountsWithUsage = applyRateLimitState(await checkAllUsage(accountsWithTokens));
    ...
    const switchLabel = result.failbackDetected ? 'Failing back' : 'Switching';
    console.error(`[claude-nonstop] ${switchLabel} to "${nextAccount.name}" (${best.reason})`);
    ...
    if (session) {
      // Use cross-profile source account + cwdHash when session was found via fallback search
      const migration = crossProfileSession
        ? migrateSessionByHash(
            crossProfileSession.account.configDir,
            nextAccount.configDir,
            crossProfileSession.cwdHash,
            session.sessionId
          )
        : migrateSession(
...
function runOnce(claudeArgs, account, existingSessionId, options = {}) {
  return new Promise((resolve) => {
    ...
    let failbackDetected = false;
    let resetTime = null;
    let outputBuffer = '';
    let lastOutputAt = Date.now();
    let pendingFailback = null;
    let failbackInterval = null;
    let failbackInFlight = false;
    const spawnedAt = Date.now();

    child.onData((data) => {
      process.stdout.write(data);
      // Only count substantive output for idle detection — ignore ANSI-only
      // chunks (cursor movement, color resets, status bar updates) that the PTY
      // emits even when Claude is waiting at the prompt.
      if (stripAnsi(data).trim().length > 0) {
      ...
      // Skip rate limit detection during grace period after spawn.
      // When resuming a session (--resume), Claude re-renders conversation history
      // which may include the previous account's rate limit message — detecting it
      // here would cause a false positive swap.
      if (Date.now() - spawnedAt < RATE_LIMIT_GRACE_MS) return;
    ...
    const maybeTriggerFailback = async () => {
      if (!options.autoFailback || rateLimitDetected || failbackDetected || failbackInFlight) return;
      if (!Array.isArray(options.allAccounts) || options.allAccounts.length < 2) return;

      failbackInFlight = true;
      try {
        const accountsWithTokens = options.allAccounts.map(a => ({
          ...a,
          token: readCredentials(a.configDir).token,
        })).filter(a => a.token);

        if (accountsWithTokens.length < 2) return;

        const accountsWithUsage = await checkAllUsage(accountsWithTokens);
        const candidate = findFailbackAccount(accountsWithUsage, account, {
          usePriority: true,
          cooldownMs: options.failbackCooldownMs ?? FAILBACK_COOLDOWN_MS,
          lastSwitchAt: options.lastSwitchAt ?? 0,
          now: Date.now(),
        });

        pendingFailback = candidate;
        if (!candidate) return;

        if (Date.now() - lastOutputAt < (options.failbackIdleMs ?? FAILBACK_IDLE_MS)) {
          return;
        }
        ...
        failbackInFlight = false;
      }
    };

    if (options.autoFailback) {
      failbackInterval = setInterval(() => {
        maybeTriggerFailback().catch(() => {});
      }, options.failbackPollMs ?? FAILBACK_POLL_MS);
    }
    ...
    child.onExit(({ exitCode }) => {
      ...
      resolve({
        ...
        failbackAccount: pendingFailback?.account?.name || null,
      });
    });
  });
}
...
export {
  ...
  AUTO_FAILBACK_ENABLED, FAILBACK_POLL_MS, FAILBACK_IDLE_MS, FAILBACK_COOLDOWN_MS,
  RATE_LIMIT_GRACE_MS,
};
